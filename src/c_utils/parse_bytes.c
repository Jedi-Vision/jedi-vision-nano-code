// Simple parser for the project-specific serialization format.
// Parses the provided example byte string into C structs and prints fields.
...
// Read little-endian int32 from buffer (unaligned-safe)
static int32_t read_int32_le(const uint8_t *p) {
    int32_t v;
    memcpy(&v, p, sizeof(int32_t));
    return v; // assumes host is little-endian (matches Python struct native on common platforms)
}
...
// Map of numeric type codes to dtype strings
static const char *NUM_TO_DTYPE[] = {
    "bool",       // 0
    "int8",       // 1
    "uint8",      // 2
    "int16",      // 3
...
// Parse the serialized buffer containing an ObjectRepData
...
int parse_object_rep(const uint8_t *buf, size_t len, ObjectRepData *out) {
    ...
    for (int idx = 0; idx < count; ++idx) {
        if (i >= len) return -1;
        if (buf[i] != '|') {
            fprintf(stderr, "Expected '|' before item %d, got 0x%02x\n", idx, buf[i]);
            return -1;
        }
        i++; // consume '|'
        ...
        if (i < len && buf[i] == '|') {
            ...
            // consume '^' and break the items loop early
            i++; // consume closing '^'
            // if there are still items expected but stream ended early, that's acceptable to break
            break;
        } else if (i >= len) {
            // truncated
            break;
        } else {
            // If we hit something else (e.g. start of tensor '$'), leave pointer as-is for next parse stage
        }
    }
